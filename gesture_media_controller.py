...
def find_contours(thresh):
    # give list of all essential boundary points
...
# Initializing the previous and current co-ordinates of the hand centroid
...
while (1):
    ...
    # only considering frame from row 0-300 and col from 300-end so that main focus is on our hands
    frame = frame[:300, 300:]
    ...
    # finding maximum contour of the thresholded area
    max_cntr = max_contour(contours)
    # finding centroid of the maximum contour
    (centroid_x, centroid_y) = centroid(max_cntr)
    if (centroid_x, centroid_y) != (-1, -1):
        # drawing a circle on the identified centroid
        frame = cv2.circle(frame, (centroid_x, centroid_y),
                           5, (255, 255, 0), -1)
        # print(cv2.contourArea(max_cntr))
        # Max contour area must be greater than thois threshold
